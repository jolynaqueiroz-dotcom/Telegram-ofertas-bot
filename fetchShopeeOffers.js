...
// Código revertido para versão estável (assinatura SHA256, /fetch, /push, AutoPush, dedupe)
...
/**
 * Variáveis esperadas (defina nos Secrets / Env):
 * - TELEGRAM_BOT_TOKEN
 * - TELEGRAM_CHAT_ID
 * - SHOPEE_APP_ID
 * - SHOPEE_APP_SECRET
 * - PAYLOAD_SHOPEE (opcional JSON string)
 * - SHOPEE_KEYWORDS ou KEYWORDS (opcional, csv)
...
// persistência para evitar repetir ofertas entre reinícios
...
async function fetchOffersPage(page = 1, keyword = "") {
  ...
  try {
    ...
    if (err?.response) {
      try {
        console.log("Shopee response data:", JSON.stringify(err.response.data).slice(0, 1000));
      } catch (e) {}
    }
  }
  return offersPage;
...
// OpenAI (opcional) - se OPENAI_API_KEY estiver configurada
async function generateOpenAICaption(productName) {
  try {
    const key = process.env.OPENAI_API_KEY;
    if (!key) return productName;

    const prompt = `Escreva uma legenda curta e persuasiva para este produto: ${productName}`;
